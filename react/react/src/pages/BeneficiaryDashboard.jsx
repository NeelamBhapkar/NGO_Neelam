...
const BeneficiaryDashboard = () => {
  ...
  const [form, setForm] = useState({ itemId: '', amountNeeded: '', description: '' });
  ...
  const hasActiveRequest = requests.some(r => ["OPEN", "PENDING"].includes(r.requestStatus));

  // --- API Calls ---
  const fetchRequests = () => {
    axios.get(`http://localhost:8084/api/beneficiary/requests/${beneficiaryId}`)
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8084/api/items").then(res => setItems(res.data));
  }, [beneficiaryId]);

  // --- Handlers ---
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!proofFile) return alert("Please upload proof document");
    ...
    if (isFinancial) formData.append("amountNeeded", form.amountNeeded);

    try {
      await axios.post("http://localhost:8084/api/beneficiary/request", formData);
      alert("Request submitted");
      setForm({ itemId: '', amountNeeded: '', description: '' }); setProofFile(null);
      fetchRequests();
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:8084/api/beneficiary/request/${id}`, formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete request?")) {
      await axios.delete(`http://localhost:8084/api/beneficiary/request/${id}`);
      fetchRequests();
    }
  };
            ...
            <div className="card border-0 shadow-sm h-100"><div className="card-header bg-white fw-bold py-3">Create Aid Request</div>
              <div className="card-body">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  {isFinancial && <div className="mb-3"><input type="number" className="form-control" placeholder="Amount Needed (₹)" value={form.amountNeeded} onChange={e => setForm({ ...form, amountNeeded: e.target.value })} required /></div>}
                  ...
                  <tbody>
                    {requests.map(r => (
                      <RequestRow key={r.requestId} r={r} isEditing={editingId === r.requestId}
                        ...
                        onEdit={() => handleEditSetup(r)} onUpdate={() => handleUpdate(r.requestId)} 
...
const RequestRow = ({ r, isEditing, editForm, setEditForm, setEditProofFile, onEdit, onUpdate, onCancel, onDelete }) => {
  ...
  const canModify = !['APPROVED', 'FULFILLED', 'CLOSED'].includes(r.requestStatus);
  ...
  if (isEditing) {
    return (
      <tr className="bg-light">
        ...
        <td>{isFinancial ? <input type="number" className="form-control form-control-sm" style={{width:80}} value={editForm.amountNeeded} onChange={e => setEditForm({ ...editForm, amountNeeded: e.target.value })} /> : "-"}</td>
  ...
  return (
    <tr>
      ...
      <td>{r.proofDocument ? <button className="btn btn-sm btn-link p-0 text-decoration-none" onClick={() => window.open(`http://localhost:8084/api/beneficiary/proof/${r.proofDocument}`, "_blank")}>View</button> : "-"}</td>
